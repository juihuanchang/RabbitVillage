...
@onready var diary_scroll: ScrollContainer = $CanvasLayer/AUI/DiaryWindow/Card/Margin/Layout/Scroll
...
func _setup_diary_layer() -> void:
	# 日記必須高於地點按鈕、建築工具列與其他 HUD，避免最新故事被介面遮住。
	_diary_layer = CanvasLayer.new()
	_diary_layer.name = "DiaryLayer"
	_diary_layer.layer = 100
	add_child(_diary_layer)
	diary_window.reparent(_diary_layer)
...
func _style_interface() -> void:
	panel.add_theme_stylebox_override("panel", UIStyleFactory.panel(Color("#fff7df"), Color("#c9ae78"), 28, 2, 12))
	hud_toggle_button.add_theme_stylebox_override("normal", UIStyleFactory.panel(Color("#fff7df"), Color("#c9ae78"), 16, 2, 6))
	hud_toggle_button.add_theme_stylebox_override("hover", UIStyleFactory.panel(Color("#fffaf0"), Color("#9e875d"), 16, 2, 7))
	...
	popup_card.add_theme_stylebox_override("panel", UIStyleFactory.panel(Color("#fffaf0"), Color("#b89559"), 28, 2, 18))
	diary_card.add_theme_stylebox_override("panel", UIStyleFactory.panel(Color("#fffaf0"), Color("#b89559"), 28, 2, 18))
	activity_status.add_theme_stylebox_override("panel", UIStyleFactory.panel(Color("#294b31e8"), Color("#88ac75"), 18, 2, 8))
	for button in [forest_button, fishing_button, start_button]:
		button.add_theme_stylebox_override("normal", UIStyleFactory.button(Color("#507e55")))
		button.add_theme_stylebox_override("hover", UIStyleFactory.button(Color("#659a68")))
		button.add_theme_color_override("font_color", Color.WHITE)
	for bar in [hunger_bar, mood_bar, energy_bar]:
		bar.add_theme_stylebox_override("background", UIStyleFactory.bar(Color("#e7ddc4")))
	hunger_bar.add_theme_stylebox_override("fill", UIStyleFactory.bar(Color("#e49b4b")))
	mood_bar.add_theme_stylebox_override("fill", UIStyleFactory.bar(Color("#df7784")))
	energy_bar.add_theme_stylebox_override("fill", UIStyleFactory.bar(Color("#66a975")))
...
func _on_countdown_changed(active: ActiveActivityData, remaining: float) -> void:
	...
	var finish := TimeManager.get_local_datetime(active.ends_at)
...
func _open_diary() -> void:
	_refresh_diary()
	diary_window.show()
	call_deferred("_scroll_diary_to_latest")


func _refresh_diary() -> void:
	for child in diary_list.get_children():
		diary_list.remove_child(child)
		child.queue_free()
	...
	if diary_window.visible:
		call_deferred("_scroll_diary_to_latest")


func _scroll_diary_to_latest() -> void:
	# 日記依時間由新到舊排列；重新開啟或收到新故事時回到最上方。
	diary_scroll.scroll_vertical = 0
...
func _make_diary_entry(entry: JournalEntry) -> Control:
	...
	var datetime := TimeManager.get_local_datetime(entry.created_at)
