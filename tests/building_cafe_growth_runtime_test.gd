...
func _test_building_and_cafe() -> void:
	...
	var rollback_currency := CurrencyManager.new(); rollback_currency.add_coins(100, "test", "seed"); var failing := FailingInventory.new(); failing.add_item("twig", 10, "test", "seed"); failing.add_item("driftwood", 5, "test", "seed"); failing.add_item("small_stone", 4, "test", "seed")
	rollback_buildings.setup_economy(rollback_currency, failing)
	_expect(not rollback_buildings.place_building("coffee_shop", "Slot01").ok and rollback_currency.get_coin_amount() == 100 and failing.get_item_amount("twig") == 10 and failing.get_item_amount("driftwood") == 5 and rollback_data.building_records.is_empty(), "Material failure after coin spend must rollback the whole placement")
	...
	var inventory := InventoryManager.new(); inventory.add_item("twig", 10, "test", "seed"); inventory.add_item("driftwood", 5, "test", "seed"); inventory.add_item("small_stone", 4, "test", "seed")
	...
	_expect(placed.ok and currency.get_coin_amount() == 20 and inventory.get_item_amount("twig") == 0 and inventory.get_item_amount("driftwood") == 0, "Placement must atomically consume full cost")
