...
func _init(
	initial_id: String = "", initial_location_id: String = "", initial_location_name: String = "",
	...
	initial_intimacy_change: int = 0, initial_reward_items: Array[String] = [],
	initial_required_energy: int = 0,
	...
	location_name = initial_location_name.strip_edges()
	...
	reward_items.assign(initial_reward_items)
...
static func create_forest_walk() -> ActivityData:
	return ActivityData.new("forest_walk", "forest", "森林", "森林散步", "和 Amy 在森林裡散步。", 30.0, -5, -3, 3, 10, 0, 1, [], 5)

static func create_forest_explore() -> ActivityData:
	return ActivityData.new("forest_explore", "forest", "森林", "森林探索", "和 Amy 深入森林探索。", 60.0, -12, -6, 5, 18, 0, 1, [], 12)

static func create_fishing() -> ActivityData:
	return ActivityData.new("fishing", "lake", "湖邊", "湖邊釣魚", "和 Amy 一起在湖邊釣魚。", 45.0, -4, -2, 4, 0, 10, 1, [], 4)

static func create_home_rest() -> ActivityData:
	return ActivityData.new("home_rest", "home", "家裡", "在家休息", "讓 Amy 在家好好休息。", 30.0, 15, -2, 2, 0, 0, 1, [], 0, true, TYPE_HOME)
...
func to_dict() -> Dictionary:
	return {
		...
		"mood_change": mood_change, "mood_reward": mood_change,
...
static func from_dict(data: Dictionary) -> ActivityData:
	...
	for item: Variant in data.get("reward_items", data.get("items", [])):
		loaded_items.append(str(item))
	var loaded_activity_id := str(data.get("activity_id", ""))
	var loaded_location_id := str(data.get("location_id", ""))
	var loaded_location_name := str(data.get("location_name", ""))
	...
	return ActivityData.new(
		...
		int(data.get("energy_change", legacy_energy)),
		int(data.get("hunger_change", 0)),
		int(data.get("mood_change", data.get("mood_reward", 0))),
		int(data.get("forest_experience_change", 0)),
		int(data.get("fishing_experience_change", 0)),
