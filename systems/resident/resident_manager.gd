...
func set_resident_location(resident_id: String, location: String) -> bool:
	...
	resident.state.current_location = location; _commit(); return true
...
func interact(resident_id: String, interaction_type: String, interaction_id := "") -> ResidentInteractionResult:
	...
	_rabbit.mood += result.mood_change; _rabbit.intimacy += result.intimacy_change; result.success = true; result.reaction_tag = get_resident_reaction_tag(resident_id, interaction_type); _commit(); resident_progress_changed.emit(resident); return result
func apply_shared_activity(active: ActiveActivityData) -> SharedActivityResult:
	...
	result.success = true; result.relationship_state = resident.relationship.state; result.reaction_tag = get_resident_reaction_tag(resident_id, active.activity.location_id); _commit(); resident_progress_changed.emit(resident); return result
...
func trigger_home_visit(resident_id: String, event_id := "resident_first_visit_001", now := -1.0) -> bool:
	...
	var resident := get_resident(resident_id); resident.daily_interactions["last_home_visit_at"] = check_time; resident.state.current_location = "home"; apply_event_relationship(resident_id, event_id); resident_event_created.emit(event_id, resident_id); _commit(); return true
func give_resident_gift(resident_id: String, event_id: String, item_id := "carrot", amount := 1) -> bool:
	...
	resident.completed_event_ids.append(event_id); _apply_source(resident, "gift", event_id, 4, 2); resident_event_created.emit(event_id, resident_id); _commit(); resident_progress_changed.emit(resident); return true
...
func create_letter(resident_id: String, event_id: String) -> Dictionary:
	...
	resident.completed_event_ids.append(event_id); resident_event_created.emit(event_id, resident_id); _commit(); return {"event_id": event_id, "resident_id": resident_id, "reaction_tag": get_resident_reaction_tag(resident_id, "letter")}
func create_invitation(resident_id: String, activity_id: String, invitation_id: String) -> Dictionary:
	...
	resident.pending_invitation = {"invitation_id": invitation_id, "activity_id": activity_id, "state": "pending"}; _commit(); return resident.pending_invitation.duplicate(true)
func respond_to_invitation(resident_id: String, invitation_id: String, accept: bool) -> Dictionary:
	...
	var result := resident.pending_invitation.duplicate(true); result["state"] = "accepted" if accept else "declined"; resident.pending_invitation = {}; _commit(); return {"ok": true, "invitation": result}
