...
func start_rest_pavilion_use() -> Dictionary:
	...
	data.active_building_interaction = active.to_dict(); building_interaction_started.emit(active); return {"ok": true, "reason": "", "active": active}
func check_rest_pavilion_completion() -> BuildingUseResult:
	...
	data.building_interactions["rest_pavilion_cooldown_ends_at"] = TimeManager.get_now() + definition.cooldown_seconds; data.active_building_interaction = {}; village.register_rest_pavilion_use(); village.claim_reward_once("first_use:rest_pavilion", 15)
