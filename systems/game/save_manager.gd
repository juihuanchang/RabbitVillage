...
func save_game()->bool:
    ...
    s.completed_activity_ids=_activity_manager.get_completed_record_ids(); s.journals=_diary_manager.to_array()
...
func load_or_create(default_rabbit:RabbitData)->RabbitData:
    ...
    _restore_activity(s,rabbit)
    # Loading also rewrites migrated data once, so obsolete fields disappear without resetting the save.
    save_game()
    return rabbit
func migrate_save_data(s:SaveData)->SaveData:
    ...
    s.village_data=v.to_dict(); s.save_version=SaveData.CURRENT_VERSION; return s
func _capture_week4(s:SaveData,r:RabbitData)->void:
    var v:=VillageData.from_dict(r.village_data) if not r.village_data.is_empty() else VillageData.create_migrated_week3_default(); s.village_data=v.to_dict()
